...
> Fix base (or radix) $\beta\ge 2$ and precision $t\ge 1$. The idealized floating point system is $F=\{0\}\ \cup\ \Bigl\{\,x\in\mathbb{R}:\ x=\pm (m/\beta^t)\beta^e,\ \beta^{t-1}\le m\le \beta^t-1,\ e,m \in\mathbb{Z}\Bigr\}$. $\pm(m/\beta^t)$ is called the fraction (or mantissa) of $x$, and $e$ is the exponent.

> [!definition|] Machine Epsilon
> The machine epsilon is $\epsilon = \frac{1}{2}\beta^{1-t}$.

> [!Theorem|]
> Let $F$ be an idealized floating point system. There exists $x’\in F$ s.t. $|x-x’|\le \epsilon_{\text{machine}}|x|$ for any $x\in \mathbb{R}$.
